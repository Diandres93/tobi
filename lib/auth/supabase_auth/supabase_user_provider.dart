...
class TobiSupabaseUser extends BaseAuthUser {
  TobiSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> tobiSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TobiSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
